...
export default async function DashboardPage() {
  ...
  const totalResult = await sql`SELECT COUNT(*) FROM notifications`
  const pendingResult = await sql`SELECT COUNT(*) FROM notifications WHERE status = 'pending'`
  const respondedResult = await sql`SELECT COUNT(*) FROM notification_responses WHERE user_id = ${userId}`
  const urgentResult = await sql`SELECT COUNT(*) FROM notifications WHERE priority = 'urgent' AND status = 'pending'`
    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card-hover stat-glow-blue bg-edp-card rounded-xl p-4 border border-blue-500/20 animate-slide-up">
          ...
          <p className="text-3xl font-display font-bold text-blue-400">{totalValue}</p>
          <p className="text-white/40 text-xs mt-1">Total Notificações</p>
        </div>
        ...
        <div className="card-hover stat-glow-orange bg-edp-card rounded-xl p-4 border border-amber-500/20 animate-slide-up">
          ...
          <p className="text-3xl font-display font-bold text-amber-400">{pendingValue}</p>
        ...
        <div className="card-hover stat-glow-green bg-edp-card rounded-xl p-4 border border-edp-green/20 animate-slide-up">
          ...
          <p className="text-3xl font-display font-bold text-edp-green">{respondedValue}</p>
          <p className="text-white/40 text-xs mt-1">Respondidas por Você</p>
        </div>
        ...
        <div className="card-hover stat-glow-red bg-edp-card rounded-xl p-4 border border-red-500/20 animate-slide-up">
          ...
          <p className="text-3xl font-display font-bold text-red-400">{urgentValue}</p>
          <p className="text-white/40 text-xs mt-1">Urgentes</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-edp-card rounded-xl border border-white/5 animate-slide-up">
          <div className="p-4 border-b border-white/5 flex items-center justify-between">
            ...
            <a href="/dashboard/notifications" className="text-edp-green text-xs hover:text-edp-light transition-colors">
              Ver todas →
            </a>
          </div>
